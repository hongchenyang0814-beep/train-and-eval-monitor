...
- The portable training layout mirrors the reference environment: `<output-root>/lora_sft/<run-id>/`, `<output-root>/ai_infra/benchmark/<run-id>/`, or `<output-root>/ai_infra/profiler/<run-id>/`. Keep `training_config.yaml`, `trainer_log.jsonl`, `run_manifest.json`, and `training_task.md` in the run directory; checkpoints use `checkpoint-<step>/`. The Monitor supports one-level and two-level category paths.
- Training records support recoverable soft hiding. Use the `隐藏` action beside a run or `隐藏记录` in the selected run's 运行健康 panel; this only persists the stable ID in `<target-dir>/training_monitor_config.json` under `hidden_experiment_ids` and never deletes the run directory, logs, or checkpoints. Enable `显示已隐藏记录` in the left sidebar, then use `恢复` to return a run to the default list.
...
- AI-operated training runs must create `run_manifest.json`, run `scripts/experiment_manifest.py validate`, and render `training_task.md` before starting. The required fields are documented in the root README; a failed validation is a hard stop for starting AI-operated training. Evaluation detail binding is separate: it can bind any run discovered by the training Monitor, displays the run date/ID plus the manifest title when available, and reports when the optional manifest/document is unavailable.
- The evaluation toolbar places manual log upload before Sync and uses a distinct color for it. The UI accepts `.log`/`.txt` (or octet-stream) raw files; the parser rejects files without recognizable task, sample, or evaluation metadata. Accepted originals are permanent under `<output-dir>/logs/<eval-task-id>/evaluation.log`, the same path used by synchronization, with `evaluation_note.md` and analysis cache; list rows carry the `自主上传` badge and optional training binding. The generated note records the source, original filename, evaluation ID, upload time, SHA-256, parser version, recognized task/sample/completed/issue counts, and the optional training binding. Binding does not require a manifest or rendered training document, although AI-operated training still does.
