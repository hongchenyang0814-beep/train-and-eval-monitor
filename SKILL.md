...
The unified Monitor exposes `/` for StreamLake evaluation-log analysis and `/train/` for the local LLaMA-Factory training monitor. The training tab reuses the original training Monitor behavior: it reads local `trainer_log.jsonl`, GPU state, explicit PIDs, checkpoints, run manifests, and recent logs. Its default configuration is `<target-dir>/training_monitor_config.json`, initially scanning `/root/output`; users may edit that runtime file for their own output roots, explicit targets, and optional Hugging Face checkpoint-upload settings. Do not copy personal training config, upload registry, PID, or log files into the skill source.
