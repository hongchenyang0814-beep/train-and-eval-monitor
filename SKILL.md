...
Use the same portable training layout as the reference environment: `<TRAINING_OUTPUT_ROOT>/lora_sft/<run-id>/` for LoRA/SFT runs, `<TRAINING_OUTPUT_ROOT>/ai_infra/benchmark/<run-id>/` for benchmark runs, and `<TRAINING_OUTPUT_ROOT>/ai_infra/profiler/<run-id>/` for profiler runs. Keep `training_config.yaml`, `trainer_log.jsonl`, `run_manifest.json`, and generated `training_task.md` in the run directory; put checkpoints below it as `checkpoint-<step>/`. Keep `run-id` as a stable timestamp or descriptive ID using only letters, numbers, underscores, and hyphens. The Monitor supports the one-level and two-level forms and treats other categories as monitor-only unless explicitly profiled.
...
### Explanation and provenance contract

Every training execution has a mandatory explanation contract. Before an AI starts a training command it must run `experiment_manifest.py init` for the run output directory (unless a manifest already exists), fill every required field, run `validate`, and stop if validation fails. The run directory must keep the machine-readable `run_manifest.json`, the generated human-readable `training_task.md`, `trainer_log.jsonl`, and `training_config.yaml` together. Required manifest fields are `schema_version` (currently `1`), `run_id`, `title`, `purpose`, `hypothesis`, `changes`, `comparison_run`, `dataset`, `model`, `config_file`, `expected_result`, `notes`, and `created_at`; `run_id` must use only letters, numbers, underscores, and hyphens, `changes` must be a non-empty string array, `dataset` and `model` must be objects, and `config_file` must be a filename for an existing file inside that run directory. For automatic checkpoint upload, the separate target rule still requires `run_id` to equal the run directory name. After changing the task or completing it, update `run_manifest.json` and run `render` again. This is a hard workflow constraint for AI-operated training, not optional UI metadata.

Evaluation logs do not require a user-authored explanation document. Platform-synchronized logs are accepted after the guarded sync rules pass. For manual uploads, the UI accepts ordinary `.log`/`.txt` files (and raw octet-stream uploads); the parser must recognize at least one task, sample, or evaluation metadata entry, otherwise the upload is rejected. A successful manual upload permanently stores the original at `<output-dir>/logs/<eval-task-id>/evaluation.log` and writes `evaluation_note.md` beside it with the source, original filename, generated evaluation ID, upload time, SHA-256, parser version, recognized task count, sample count, completed task count, parse issue count, and optional training binding. The parsed result is also persisted under `<output-dir>/analysis_cache`, and the list shows the `自主上传` source badge; platform sync never removes these records. The detail header can bind either source to a discovered training run. Binding does not require a `run_manifest.json` or `training_task.md`; it saves the selected run identity and shows its date/ID and title when available. Missing or incomplete training documentation is reported in the evaluation page, but it does not reject the binding. Only AI-operated training launch is blocked by missing or invalid training documentation.
...
The training list supports recoverable record hiding. Each run has a visible `隐藏` action next to it, and the selected run also exposes `隐藏记录` in the 运行健康 panel. Hiding only adds the stable run ID to `hidden_experiment_ids` in `<target-dir>/training_monitor_config.json`; it never deletes the output directory, `trainer_log.jsonl`, checkpoints, or other training files. Enable `显示已隐藏记录` in the left sidebar to inspect hidden runs and use `恢复` / `恢复记录` to return them to the default list.
